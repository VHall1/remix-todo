					...
					<div className="ml-auto flex items-center gap-3">
						<Popover>
							...
							<PopoverContent className="grid">
								<Link to="." prefetch="render" className="flex items-center gap-2 py-1.5">
									{location.pathname.endsWith("/todos") ? <CheckIcon className="w-4 h-4" /> : null}
									All
								</Link>
								<Link to="active" prefetch="render" className="flex items-center gap-2 py-1.5">
									{location.pathname.endsWith("/active") ? <CheckIcon className="w-4 h-4" /> : null}
									Active
								</Link>
								<Link to="completed" prefetch="render" className="flex items-center gap-2 py-1.5">
									{location.pathname.endsWith("/completed") ? <CheckIcon className="w-4 h-4" /> : null}
									Completed
								</Link>
							</PopoverContent>
						</Popover>
						<Button size="sm" onClick={() => setOpen(true)} className="noscript-hidden">
